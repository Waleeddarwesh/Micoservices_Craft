const UsersLinuxPage = (() => {
    function render(container) {
        container.innerHTML = `
            <div class="page-header">
                <div>
                    <h2>${window.t('Linux User Administration')}</h2>
                    <p class="text-secondary">${window.t('Manage system user accounts, groups, and sudo access.')}</p>
                </div>
                <div class="header-actions">
                    <button class="btn btn-primary" onclick="Toast.show('Add user functionality not implemented yet.', 'info')">
            ...
            <div class="card">
                <div class="table-actions">
                    <div class="form-search" style="width: 320px; max-width: 100%;">
                        ...
                        <input type="text" class="form-input" placeholder="${window.t('Search...')}" oninput="UsersLinuxPage.loadData()">
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table" id="users-table">
                        <thead>
                            <tr>
                                <th>${window.t('Username')}</th>
                                <th>${window.t('Server')}</th>
                                <th>${window.t('Group')}</th>
                                <th>${window.t('Sudo Access')}</th>
                                <th>${window.t('Status')}</th>
                                <th>${window.t('Actions')}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr><td colspan="6" class="text-center py-4"><div style="display:flex;flex-direction:column;align-items:center;padding:32px 0;"><div class="spinner" style="margin-bottom:16px;"></div><p style="color:var(--clr-text-muted)">Loading users...</p></div></td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    async function loadData() {
        try {
            const data = await API.get('/api/system-admin/linux-users/');
    ...
    <div class="empty-state" style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:32px 0;">
        ...
        <p style="color:var(--clr-text-muted);font-size:var(--fs-sm);">${window.t('No linux users found.')}</p>
    </div>
</td></tr>`;
                return;
            }
            ...
            tbody.innerHTML = data.map(user => `
                <tr>
                    ...
                    <td>${user.server ? `Server #${user.server}` : 'Local'}</td>
                    <td>${user.group}</td>
                    <td>
                        <span class="badge badge-${user.sudo_access ? 'success' : 'danger'}">${user.sudo_access ? 'Yes' : 'No'}</span>
                    </td>
                    <td>
    ...
    <div class="empty-state" style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:32px 0;">
        ...
        <p style="color:var(--clr-text-muted);font-size:var(--fs-sm);">${window.t('Failed to load users')}</p>
    </div>
</td></tr>`;
        }
    }
