...
def send_single_notification_on_save(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if (not prefs or prefs.push_notifications) and user.fcm_token:
            send_push_notification(user.fcm_token, "New Notification", instance.message)
...
def send_bulk_notifications(sender, notifications, **kwargs):
    ...
    for notification in notifications:
        ...
        prefs = getattr(user, 'notification_preferences', None)
        if (not prefs or prefs.push_notifications) and user.fcm_token:
            send_push_notification(user.fcm_token, "New Notification", notification.message)