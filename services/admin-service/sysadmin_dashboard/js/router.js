...
const Router = (() => {
    ...
    function getCurrentRoute() {
        return window.location.hash.slice(1) || 'servers';
    }
        ...
        if (route !== 'servers') {
            breadcrumbHtml = `
                <div class="breadcrumbs" style="font-size:var(--fs-xs); color:var(--clr-text-muted); margin-bottom:var(--space-4); display:flex; gap:var(--space-2); align-items:center;">
                    <a href="#servers" style="color:var(--clr-primary); text-decoration:none;" onclick="Router.navigate('#servers');return false;">${window.t('Dashboard')}</a>
