        ...
        if user.is_authenticated and hasattr(user, 'supplier'):
            ...
            if not supplier.SupplierContract and not supplier.SupplierIdentity:
                create_notification_for_user(
                    user=user,
                    message=_("Upload your contract and identity documents, please.")
                )
                return False
            
            # Check if the supplier is marked as accepted
            if not supplier.accepted_supplier:
                raise ValidationError(_("Your supplier account has not been accepted yet.the adminstrators will accept your documents soon "))
    ...
    def has_permission(self, request, view):
        ...
        if view.action == 'create':
            ...
            if hasattr(user, 'supplier') and Address.objects.filter(user=user).exists():
                return True
            raise ValidationError(_("You must provide a business address before adding a product."))
