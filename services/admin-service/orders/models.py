...
class WishlistItem(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['wishlist', 'product'], name='unique_wishlist_item')
        ]
...
class DeliveryBatch(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    delivery_person = models.ForeignKey(Delivery, on_delete=models.SET_NULL, null=True, blank=True, related_name='batches')
...
class Shipment(models.Model):
    ...
    def __str__(self):
        parent_id = self.order.id if self.order else 'Unknown'
...
class ShipmentItem(models.Model):
    ...
    def __str__(self):
        product_name = self.order_item.product.ProductName if self.order_item else 'Unknown'
