...
from Handcrafts.business_config import calculate_cashback, GOLD_TIER_DELIVERY_BONUS, SILVER_TIER_DELIVERY_BONUS
from orders.services import get_craft_user_by_email
...
class WishlistViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if existing_whishlist:
            raise ValidationError(_("A wishlist already exists for this user"))
        serializer.save(user=user)
        return Response({"message": _("Wishlist created successfully")}, status=status.HTTP_201_CREATED)
...
class CartViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if existing_cart:
            raise ValidationError(_("A cart already exists for this user"))
        serializer.save(User=user)
...
class CartItemViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.CartID.User != request.user:
            return Response(
                {"message": _("You cannot delete items from another user's cart.")},
...
class OrderViewSet(mixins.CreateModelMixin, mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def retrieve_supplier_order(self, request, pk=None):
        try:
            ...
            return Response({"message": _("Order not found or you don't have permission to view it.")},
    ...
    def ready_to_ship(self, request, pk=None):
        ...
        try:
            ...
            return Response({"message": _("Shipment not found or is not in a 'created' state.")},
                            status=status.HTTP_404_NOT_FOUND)
        ...
        return Response({"message": _("Shipment for order %(pk)s is now ready to ship.") % {'pk': pk}}, status=status.HTTP_200_OK)
        ...
        try:
            ...
            if order.status in [Order.OrderStatus.CREATED, Order.OrderStatus.In_Transmit]:
                with transaction.atomic():
                    ...
                    cashback_amount = calculate_cashback(order.final_amount)
                    user = request.user
                ...
                return Response({"message": _("Order has been cancelled.")}, status=status.HTTP_200_OK)
            else:
                return Response(
                    {"message": _("Cannot cancel an order that has been delivered or marked as failed delivery.")},
                    status=status.HTTP_400_BAD_REQUEST)

        except Order.DoesNotExist:
            return Response({"message": _("Order not found.")}, status=status.HTTP_404_NOT_FOUND)
    ...
    def calculate_totals(self, request, *args, **kwargs):
        ...
        return Response({
            "message": _("Order totals calculated successfully"),
            **totals
        }, status=status.HTTP_200_OK)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if payment_method == Order.PaymentMethod.CREDIT_CARD:
            return Response({
                "message": _("Redirecting to payment gateway..."),
...
class ShipmentViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    filter_backends = [SearchFilter]
    search_fields = ['order__order_number', 'supplier__user__email', 'to_state']
    ...
    def accept(self, request, pk=None):
        try:
            ...
            return Response({'message': _("Shipment not found or is already taken.")}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response({'status': _("Shipment accepted and status updated to on my way")})
    ...
    def delivered(self, request, pk=None):
        try:
            ...
            return Response({'message': _("Shipment not found or you are not assigned to it.")},
                            status=status.HTTP_404_NOT_FOUND)

        confirmation_code = request.data.get('confirmation_code')
        if confirmation_code != shipment.confirmation_code:
            return Response({"message": _("Invalid confirmation code.")}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({'message': _("Shipment status updated to delivered successfully")}, status=status.HTTP_200_OK)
...
from .serializers import DeliveryBatchSerializer
from decimal import Decimal

class DeliveryBatchViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        if not hasattr(user, 'delivery'):
            return DeliveryBatch.objects.none()
            
        return DeliveryBatch.objects.filter(
            status__in=[DeliveryBatch.BatchStatus.PENDING, DeliveryBatch.BatchStatus.ASSIGNED, DeliveryBatch.BatchStatus.IN_PROGRESS]
        ).filter(
            Q(delivery_person__isnull=True) | Q(delivery_person=user.delivery)
        ).distinct()

    @action(detail=True, methods=['post'], url_path='accept')
    def accept_batch(self, request, pk=None):
        user = request.user
        if not hasattr(user, 'delivery'):
            return Response({'message': 'Only delivery personnel can accept batches'}, status=status.HTTP_403_FORBIDDEN)
            
        try:
            batch = DeliveryBatch.objects.get(pk=pk, status=DeliveryBatch.BatchStatus.PENDING, delivery_person__isnull=True)
        except DeliveryBatch.DoesNotExist:
            return Response({'message': 'Batch not found or already accepted'}, status=status.HTTP_404_NOT_FOUND)
            
        with transaction.atomic():
            batch.delivery_person = user.delivery
            batch.status = DeliveryBatch.BatchStatus.ASSIGNED
            
            bonus_pct = Decimal('0.0')
            tier = user.delivery.performance_tier
            if tier == user.delivery.PerformanceTier.GOLD:
                bonus_pct = GOLD_TIER_DELIVERY_BONUS
            elif tier == user.delivery.PerformanceTier.SILVER:
                bonus_pct = SILVER_TIER_DELIVERY_BONUS
                
            batch.total_base_payout = batch.total_base_payout * (Decimal('1.0') + bonus_pct)
            batch.save()
            
            shipments = batch.shipments.all()
            for shipment in shipments:
                shipment.delivery_person = user.delivery
                shipment.status = Shipment.ShipmentStatus.ON_MY_WAY
                shipment.save()
            ...
            return Response({'message': 'Batch accepted successfully', 'total_payout': batch.total_base_payout})
