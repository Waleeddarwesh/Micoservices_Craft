...
class AddWishlistItemSerializer(serializers.ModelSerializer):
    ...
    def validate_product_id(self, value):
        ...
        if not product:
            raise serializers.ValidationError(_("There is no product associated with the given ID"))
        return value
    ...
    def save(self, **kwargs):
        ...
        if product.supplier.user == user:
            raise serializers.ValidationError(_("You cannot add your own product to the wishlist"))
...
class AddCartItemSerializer(serializers.ModelSerializer):
    ...
    def validate_product_id(self, value):
        product = Product.objects.filter(id=value).first()
        if not product:
            raise serializers.ValidationError({"detail": _("There is no product associated with the given ID")})
        return value
    ...
    def save(self, **kwargs):
        ...
        if product.Supplier.user == user:
            raise serializers.ValidationError({"detail": _("You cannot add your own product to the cart")})
        
        if quantity <= 0 or quantity > 10:
            raise serializers.ValidationError({"detail": _("Quantity must be above 0 and less than or equal to 10")})
        if quantity > product.Stock :
            raise serializers.ValidationError({"detail": _("Quantity of %(name)s exceeds available stock.") % {'name': product.ProductName}})
        
        try:
            cart_item = CartItems.objects.get(Product_id=Product_id, CartID=cart, Color=color, Size=size)
            if cart_item.Quantity + quantity > product.Stock:
                raise serializers.ValidationError({"detail": _("Adding this quantity of %(name)s exceeds available stock.") % {'name': product.ProductName}})
...
class CouponSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data['valid_from'] >= data['valid_to']:
            raise serializers.ValidationError(_("Valid from date must be before valid to date."))
        
        if data.get('discount_type') == 'percentage' and data.get('discount') > 100:
            raise serializers.ValidationError(_("Percentage discount cannot be more than 100."))
    ...
    def create(self, validated_data):
        ...
        if not user.is_authenticated or not hasattr(user, 'supplier'):
            raise serializers.ValidationError(_("User is not a supplier."))
...
class DeliveryBatchSerializer(serializers.ModelSerializer):
    ...
    shipments_count = serializers.IntegerField(source='shipments.count', read_only=True)

    class Meta:
        model = DeliveryBatch
        fields = ['id', 'target_state', 'status', 'total_base_payout', 'shipments_count', 'created_at']
