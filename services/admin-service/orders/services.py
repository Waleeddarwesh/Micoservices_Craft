...
from Handcrafts.business_config import (
    calculate_product_split,
    calculate_delivery_split,
    calculate_cashback,
    INTER_STATE_SHIPPING_SURCHARGE,
)
...
def _get_supplier_addresses_helper(cart_items, user):
    ...
    for supplier_id in supplier_ids:
        ...
        if not addresses.exists() or addresses.count() > 1:
            raise ValidationError({"message": _("Address not found or multiple addresses found for supplier with ID %(id)s.") % {'id': supplier_id}})
